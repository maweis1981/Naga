...
@app.get("/traces")
def traces_list(n: int = 50):
    return {"traces": tracer.recent(n)}


@app.get("/trace/{tid}")
def trace_get(tid: str):
    d = tracer.get(tid)
    if d is None:
...
async def chat_completions(req: Request):
    ...
    if manager.settings.get("mcp_enabled") and _toolset.tools():
        ...
        _trace = tracer.start(query, meta={"model": model_name, "path": "agent"})

        def pieces():
            tracer.bind(_trace); _out = []
