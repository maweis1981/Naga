...
- **OpenAI-compatible serving** — `/v1/chat/completions` with SSE streaming, standard function calling (pass `tools` → get structured `tool_calls` back, honoring `tool_choice: auto | required | {function}` via constrained decoding), context-length guard (clean `context_length_exceeded` 400 like OpenAI), plus a self-built WebUI.
- **Multi-conversation chat UI + session management** — the self-built WebUI has a conversation sidebar (new / switch / delete), and chats persist to `~/.naga/conversations/` (one JSON per chat) via a `GET/POST/PUT/PATCH/DELETE /api/chats` API, so history survives page refresh and restart. Inference stays stateless (`/v1/chat/completions`); the store is pure CRUD.
- **Local memory + RAG** — hand-written BERT embeddings, semantic retrieval, and document (txt/md/pdf) chunking & retrieval.
- **MCP agent** — MCP client over **stdio** (local subprocess, with per-call timeouts so a stalled server can't hang a request) **or Streamable HTTP** (remote/hosted servers, JSON or SSE responses), plus a multi-step tool-calling loop (`tool_choice: auto | required | none`). The agent decomposes a task and dispatches to different tools across steps, and executes **multiple tool calls in a single turn** (parallel tool calls, aligned with OpenAI's `tool_calls` array). **Intent-based tool routing**: MCP tools are discovered, embedded (with the self-built BGE embedder) and indexed to `~/.naga/tool_index.json`; when many tools are connected, the user's message is matched semantically so only the most relevant top-k tools are handed to the model (instead of dumping all of them into the prompt) — faster prefill and better small-model decisions. Config in `~/.naga/mcp.json`: `{"mcpServers": {"local": {"command": "python3", "args": [...]}, "remote": {"url": "https://host/mcp", "headers": {"Authorization": "Bearer ..."}}}}`.
- **Agent SDK** — an importable `Agent` class + `@tool` decorator (`naga.sdk`) that turns plain Python functions into tools (JSON schema auto-derived from type hints) and runs the constrained tool-calling loop locally, composable with MCP servers. Includes an agent-runtime control plane inspired by the Claude Agent SDK: **permissions** (`allowed_tools` / `blocked_tools` / a `can_use_tool(name, args)` callback that can allow / deny-with-reason / rewrite input), **hooks** (`on_tool_call` / `on_tool_result`), **subagent delegation** (`agent.as_tool()` — a specialized sub-agent a main agent can call), and **usage** in `AgentResult` (`tool_calls` / `denied` / `steps` / `completion_tokens`). See `examples/agent_sdk_demo.py` and `examples/agent_controls_demo.py`.
